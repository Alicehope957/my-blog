...
(async () => {
  ...
  // 让「点击」也能播放动作
  // widget 自带逻辑里，只有 mousemove 会去播 Motions.TapBody 组的动作（onMouseMove 内部会调
  // lapplive2dmanager.onTap），而 pointerdown 只负责派发 live2d:tapbody 弹文案、不播动作。
  // 这里在"点击到模型本体"时补发一次 mousemove，复用 widget 自己的处理链，
  // 不需要修改任何 CDN 上的代码。
  document.addEventListener('pointerdown', event => {
    const target = event.target;
    if (!target) return;
    const onModel = target.id === 'live2d' ||
      (target.closest && target.closest('#waifu-canvas'));
    if (!onModel) return;
